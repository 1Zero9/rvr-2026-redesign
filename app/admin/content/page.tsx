...
function NoticeCard({ r }: { r: NoticeRow }) {
  return (
    <div
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          {r.live && (
            <span className="text-[10px] font-bold text-brand-green uppercase tracking-wide">● Live</span>
          )}
          {r.scheduled && r.isPublished && (
            <span className="text-[10px] font-bold text-brand-sky uppercase tracking-wide">Scheduled</span>
          )}
          {r.expired && (
            <span className="text-[10px] font-bold text-brand-maroon uppercase tracking-wide">Expired</span>
          )}
          {!r.isPublished && (
            <span className="text-[10px] font-bold text-brand-charcoal/40 uppercase tracking-wide">Draft</span>
          )}
        </div>
...
export default async function ContentAdminPage() {
  ...
  const campaignRows: NoticeRow[] = campaigns.map((c): NoticeRow => {
  ...
  const newsRows: NoticeRow[] = announcements.map((a): NoticeRow => {
  ...
  const rows = [...campaignRows, ...newsRows];
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-brand-charcoal/60 text-sm mt-1">
              {rows.length} items · {liveCount} live now — news and campaigns, and
              exactly where each one shows on the site.
            </p>
          </div>
          <div className="flex gap-2">
            <Link
              href="/admin/noticeboard/new?type=news"
              className="shrink-0 bg-white text-brand-navy font-bold px-4 py-3 min-h-[44px] flex items-center border-3 border-brand-navy hover:bg-brand-navy hover:text-brand-cream transition-all"
            >
              + News
            </Link>
            <Link
              href="/admin/noticeboard/new?type=campaign"
              className="shrink-0 bg-brand-neon text-brand-charcoal font-bold px-4 py-3 min-h-[44px] flex items-center border-3 border-brand-charcoal shadow-brutalist hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all"
            >
              + Campaign
            </Link>
          </div>
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between gap-2 mb-3">
            <h2 className="font-display font-black uppercase text-sm text-brand-navy tracking-wide">
              News <span className="font-normal text-brand-charcoal/40">({newsRows.length})</span>
            </h2>
            <Link href="/admin/noticeboard/new?type=news" className="text-xs font-bold text-brand-navy hover:underline">
              + Add news
            </Link>
          </div>
          {newsRows.length === 0 ? (
            <div className="bg-white border-2 border-brand-charcoal/10 p-6 text-center text-sm text-brand-charcoal/50">
              No news items yet.
            </div>
          ) : (
            <div className="space-y-3">
              {newsRows.map((r) => <NoticeCard key={`${r.kind}-${r.id}`} r={r} />)}
            </div>
          )}
        </section>
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between gap-2 mb-3">
            <h2 className="font-display font-black uppercase text-sm text-brand-navy tracking-wide">
              Campaigns &amp; Events <span className="font-normal text-brand-charcoal/40">({campaignRows.length})</span>
            </h2>
            <Link href="/admin/noticeboard/new?type=campaign" className="text-xs font-bold text-brand-navy hover:underline">
              + Add campaign
            </Link>
          </div>
          <p className="text-xs text-brand-charcoal/50 mb-3">
            Banners, hero background, and the highlight badge are all just placements
            on a campaign — see the chips below, or the placement panel above.
          </p>
          {campaignRows.length === 0 ? (
            ...
            <div className="space-y-3">
              {campaignRows.map((r) => <NoticeCard key={`${r.kind}-${r.id}`} r={r} />)}
            </div>
          )}
        </section>
