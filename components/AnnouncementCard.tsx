...
export default function AnnouncementCard({
  ...
  return (
    <Link
      href={dest}
      className="group bg-white border-2 border-brand-charcoal/15 hover:border-brand-navy/50 flex flex-col overflow-hidden h-full transition-colors"
    >
      <div className={`h-1.5 shrink-0 ${style.strip}`} />
      ...
      <div className="p-5 flex-1 flex flex-col gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {a.pinned && (
            <span className="text-[10px] font-bold text-brand-charcoal/40 uppercase tracking-wide">Pinned</span>
          )}
        ...
        <h3 className="font-display font-black text-base leading-snug text-brand-charcoal group-hover:text-brand-navy transition-colors">
          {a.title}
        </h3>

        <p className="text-xs text-brand-charcoal/50 leading-relaxed line-clamp-3 flex-1">
      ...
      {a.ctaLabel && (
        <div className="px-5 pb-5 pt-1">
          <span className="inline-flex items-center gap-1 text-xs font-bold text-brand-navy group-hover:underline underline-offset-2">
            {a.ctaLabel} →
          </span>
        </div>
      )}
